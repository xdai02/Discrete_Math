...
命题$ p \rarr q $表示“$ p $蕴含$ q $”，在$ p $为真而$ q $为假时命题为假，否则为真。其中$ p $称为前提，$ q $称为结论。
...
$ p \rarr q $：如果我去看电影，那么我会买奶茶。

---

![](./img/C1/1-2/1.png)

由$ p \rarr q $可以构造出几个相关的蕴含：

1. $ q \rarr p $称为$ p \rarr q $的逆命题（converse）。
2. $ \neg q \rarr \neg p $称为$ p \rarr q $的逆否命题（contrapositive）。
...
$ p \rarr q $：如果今天是星期四，那么我今天有考试。

$ q \rarr p $：如果我今天有考试，那么果今天是星期四。
...
命题$ p \lrarr q $表示“$ p $双向蕴含$ q $”，在$ p $和$ q $有相同的真值时命题为真，否则为假。

$ p \lrarr q $等价于$ (p \rarr q) \wedge (q \rarr p) $。
...
双蕴含的真值表与异或的真值表正好相反，因此$ p \lrarr q $与$ \neg (p \oplus q) $等价。
...
如果复合命题$ s $和是$ r $逻辑等价的，可表示为$ s \equiv r $。只有当$ s \lrarr r $是永真式时，$ s $和$ r $才是逻辑等价的。
...
| 条件恒等                      | $$ p \rarr q \equiv \neg p \vee q \\ p \lrarr q \equiv (p \rarr q) \wedge (q \rarr p) $$ |

---

【范例】证明$ (p \vee q) \rarr p $永真
$$
\begin{aligned}
& (p \vee q) \rarr p \\
& \equiv \neg (p \wedge q) \vee p \\
...
量词有全称量词（universal quantifier）和存在量词（existential quantifer）：
...
【范例】$ P(x) $：$ x $是偶数，$ Q(x) $：$ x $能被3整除，$ x \in \Z^+ $
